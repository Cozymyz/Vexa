...
<div class="shop-view">
  ...
  <div class="shop-grid">
    <div
      v-for="shop in sortedAndFilteredShops"
      :key="shop.shopId"
      class="shop-card"
    >
      <h3>{{ shop.shopName }}</h3>
      <p>{{ shop.shopIntroduce }}</p>
      <button @click="viewShopDetail(shop)">Detail</button>
    </div>
  </div>
</div>
</template>
...
const fetchReviewList = async (skipIfExists = false) => {
    const moduleName = 'ReviewList'
    // Skip existing modules
    if (store.hasModule(moduleName)) {
        if (skipIfExists) {
            return { success: true, skipped: true, reason: 'The module already exists' }
    ...
    try {
        await waitForModule(moduleName)
        await store.dispatch(`${moduleName}/fetchReviewList`)
    } catch (error) {
        console.error(`Error loading module ${moduleName}: ${error.message}`)
        throw error
    }
...
const availableGenres = ref<GenreList[]>([]) // Fetch data from the module
const localShops = ref<ShopList[]>([]) // Fetch data from the module

function toggleSortOrder() {
  sortAscending.value = !sortAscending.value
}
function viewShopDetail(shop: ShopList) {
  console.log('Detail:', shop)
  // Jump to dialog
}

const sortedAndFilteredShops = computed(() => {
  let list = [...localShops.value]
